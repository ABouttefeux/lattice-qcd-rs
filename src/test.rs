...
fn test_thread() {
    ...
    for number_of_thread in &[1, 2, 4] {
        let result = run_pool_parallel(iter.clone(), &c, &|i, c| {i * i * c} , *number_of_thread, 10000).unwrap();
        for i in iter.clone() {
...
fn test_thread_vec() {
    ...
    let c = 5;
    for number_of_thread in &[1, 2, 4] {
        let result = run_pool_parallel_vec(iter.clone(), &c, &|i, c| {i * i * c}, *number_of_thread, 10000, &l, &0).unwrap();
        for i in iter.clone() {
            assert_eq!(*result.get(i).unwrap(), i * i *c);
        }
    }
...
fn test_sim_cold(){
    ...
    let sim3 = sim2.simulate_leap(&SymplecticEulerRayon::new(), 0.1).unwrap();
    assert_eq!(sim2.e_field(), sim3.e_field());
    assert_eq!(sim2.link_matrix(), sim3.link_matrix());
    let sim4 = sim3.simulate_to_synchrone(&SymplecticEulerRayon::new(), 0.1).unwrap();
