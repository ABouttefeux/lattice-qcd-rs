    ...
    Direction<D>: DirectionList,
{
    ...
    fn integrate_symplectic(&self, l: &State, delta_t: Real) -> Result<State, SimulationError> {
        ...
        let e_field_demi = EField::new(get_e_field_integrate::<State, D>(l.link_matrix(), l.e_field(), l.lattice(), self.number_of_thread, delta_t / 2_f64)?);
        let link_matrix = LinkMatrix::new(get_link_matrix_integrate::<State, D>(l.link_matrix(), &e_field_demi, l.lattice(), self.number_of_thread, delta_t)?);
        // we advace the counter by one
        let e_field = EField::new(get_e_field_integrate::<State, D>(&link_matrix, &e_field_demi, l.lattice(), self.number_of_thread, delta_t / 2_f64)?);
