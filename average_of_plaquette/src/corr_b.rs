...
fn main_cross_with_e(simulation_index: usize) {
    ...
    */
    ...
    let _ = write_vec_to_file_csv(&measure, &format!("{}/mean_measures_corr_e_{}.csv", DIRECTORY, beta));
    ...
    let _ = plot_data_fft(&measure_fft[..measure_fft.len() / 2], DT, &format!("{}/e_corr_{}_fft.svg", DIRECTORY, beta));
    let _ = plot_data_fft_2(&measure_fft[..measure_fft.len() / 2], DT, &format!("{}/e_corr_{}_fft_2.svg", DIRECTORY, beta));
    pb.inc(1);
