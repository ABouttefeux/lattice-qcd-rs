...
pub fn plot_data(data: &[[f64; 2]], delta_t: f64, file_name: &str) -> Result<(), Box<dyn std::error::Error>> {
...
pub fn plot_data_fft(data: &[Complex<f64>], delta_t: f64, file_name: &str) -> Result<(), Box<dyn std::error::Error>> {
...
pub fn plot_data_fft_2(data: &[Complex<f64>], delta_t: f64, file_name: &str) -> Result<(), Box<dyn std::error::Error>> {
    let mut y_min = data[0].modulus() / (data.len() as f64).sqrt();
    let mut y_max = data[0].modulus() / (data.len() as f64).sqrt();
    for el in data {
        y_min = y_min.min(el.modulus() / (data.len() as f64).sqrt());
