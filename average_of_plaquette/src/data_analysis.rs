...
impl AverageData {
    ...
    fn compute_average(&mut self) -> Real {
        let av =
            self.data.par_iter().map(|el| el.average()).sum::<Real>() / (self.data.len() as f64);
...
pub fn plot_data_average(data: &[(Config, AverageData)]) -> Result<(), Box<dyn std::error::Error>> {
    let betas = data
        .iter()
        .map(|(cfg, _)| cfg.lattice_config().lattice_beta())
        .collect::<Vec<f64>>();
    let avg = data
        .iter()
        .map(|(_, avg)| avg.clone().final_average())
        .collect::<Vec<f64>>();
...
fn plot_data_beta(betas: &[f64], avg: &[f64]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    chart.draw_series(
        betas
            .iter()
            .zip(avg.iter())
            .map(|(beta, avg)| Circle::new((*beta, *avg), 2, BLACK.filled())),
    )?;
    Ok(())
}
...
pub fn plot_data_auto_corr(
    ...
    chart.draw_series(
        auto_corr
            .iter()
            .enumerate()
            .map(|(x, y)| Circle::new((x as f64, *y), 2, BLACK.filled())),
    )?;
    Ok(())
}

/// save configuration to `format!("sim_b_{}.bin", cfg.lattice_config().lattice_beta())`
pub fn save_data<const D: usize>(
    cfg: &Config,
...
pub fn save_data_n<const D: usize>(
    cfg: &Config,
    state: &LatticeStateDefault<D>,
    sufix: &str,
) -> std::io::Result<()> {
