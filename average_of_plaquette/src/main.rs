...
fn main_cross_check_volume() {
    ...
    let beta = 24_f64;
    let mut vec_dim = vec![];
    let number_of_data = 1;
    let mut therm_setp = vec![];
    
    for i in 0..number_of_data {
        //let n: usize = (beta / get_values(1.0, 2.5, i, number_of_data)).round() as usize;
        let n: usize = 10;
        vec_dim.push(n);
        therm_setp.push( n.pow(4) * 5_000);
    }
    ...
    let sim_cfg = SimConfigScan::new(
        ...
        ScanPossibility::Default(5_000), // renormn
        ScanPossibility::Default(100), // number_of_averages
...
fn main_scan_beta() {
    ...
    let result = array_config.par_iter().enumerate().map(|(index, cfg)| {
        ...
        let sim_init = generate_state_default(cfg.lattice_config(), &mut rng);
        let (av, sim_final, _) = run_simulation_with_progress_bar_average(cfg.sim_config(), sim_init, &multi_pb, rng);
        let _ = save_data(cfg, &sim_final);
        pb.inc(1);
        (*cfg, av)
    }).collect();
