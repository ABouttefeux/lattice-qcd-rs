...
fn main() {
    ...
    let data = N_ARRAY.par_iter().map( |n_size| {
        ...
        let (mean_and_err_block, mean_and_err_individual, mean_and_err_mean) = thread::scope(|s| {
            ...
            (mean_and_err_block, mean_and_err_individual, mean_and_err_mean)
        }).unwrap();
        pb.inc(1);
...
fn statistical_boot_strap_method_block(data: &[Vec<f64>], beta: f64, number_of_times: usize, rng: &mut impl Rng ) -> [f64; 2] {
    let size = data.len();
    ...
    for _ in 0..number_of_times {
        ...
        for _ in 0..size{
            ...
            let mut data_s = data[pos_1].iter().map(|el| observable::parameter_volume(*el, beta)).collect();
...
fn statistical_boot_strap_method_individual(data: &[Vec<f64>], beta: f64, number_of_times: usize, rng: &mut impl Rng ) -> [f64; 2] {
...
fn plot_data(data: &[(usize, [f64;2])], beta: f64, file_name: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut chart = ChartBuilder::on(&root)
        ...
        .build_cartesian_2d(
            0_f64..2.6,
            (y_min - err_max).min(0_f64)..y_max + err_max,
        )?;
    ...
    chart.draw_series(data.iter().map(|(n, [mean, err])| {
        ErrorBar::new_vertical(beta / (*n as f64), mean - err, *mean , mean + err, BLACK.filled(), 10)
    }))?;
    Ok(())
}
