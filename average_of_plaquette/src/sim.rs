...
use lattice_qcd_rs::{
    error::{MultiIntegrationError, StateInitializationError},
    field::Su3Adjoint,
...
use super::{config::*, data_analysis, data_analysis::*, observable};
...
pub fn generate_state_default<Rng: rand::Rng, const D: usize>(
    cfg: &LatticeConfig,
    rng: &mut Rng,
    ...
    .expect("Invalide Configuration")
}

/// Generate a [`MetropolisHastingsDeltaDiagnostic`] from a config
pub fn get_mc_from_config<Rng>(
    cfg: &MonteCarloConfig,
    rng: Rng,
) -> MetropolisHastingsDeltaDiagnostic<Rng>
where
    Rng: rand::Rng,
{
...
pub fn get_mc_from_config_sweep<Rng>(
    cfg: &MonteCarloConfig,
    rng: Rng,
) -> MetropolisHastingsSweep<Rng>
where
    Rng: rand::Rng,
{
    MetropolisHastingsSweep::new(cfg.number_of_rand(), cfg.spread(), rng)
        .expect("Invalide Configuration")
}
    ...
    Rng: rand::Rng,
{
    ...
    let pb_th = mp.add(ProgressBar::new(
        (config.number_of_averages() * config.number_of_steps_between_average()
            + config.number_of_thermalisation()) as u64,
    ));
    pb_th.set_style(
    ...
    F: Fn(&LatticePoint<D>, &LatticeStateDefault<D>) -> f64 + Sync,
{
    ...
    let measurement_val_init = points
        .par_iter()
    ...
    let mut measurement_val = points
        .par_iter()
        .map(|el| observable(el, &state))
        .sum::<f64>();
    ...
    let _ = data_analysis::plot_data_auto_corr(
        &vec_corr_plot,
        &format!(
            "{}plot_auto_corr_{}_{}.svg",
            prefix,
            state.lattice().dim(),
            sufix
        ),
    ...
    F: Fn(&LatticePoint<D>, &LatticeStateDefault<D>) -> f64 + Sync,
{
    ...
    let pb = mp.add(ProgressBar::new(
        (number_of_measurement * number_of_discard) as u64,
    ));
    pb.set_style(
        ProgressStyle::default_bar()
            .progress_chars("=>-")
