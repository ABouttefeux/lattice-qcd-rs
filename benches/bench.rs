...
fn bench_simulation_creation(size: usize, rng: &mut rand::rngs::ThreadRng, d: &impl rand_distr::Distribution<Real>){
    let _simulation = LatticeSimulation::new(1_f64 , size, rng, d).unwrap();
}
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("sim creation 20", |b| b.iter(|| bench_simulation_creation(20, &mut rng, &d)));
    c.bench_function("sim creation 10", |b| b.iter(|| bench_simulation_creation(10, &mut rng, &d)));
    c.bench_function("matrix exp old", |b| b.iter(|| matrix_exp_old(&mut rng, &d)));
    c.bench_function("matrix exp new i ", |b| b.iter(|| matrix_exp_i(&mut rng, &d)));
    c.bench_function("matrix exp new r", |b| b.iter(|| matrix_exp_r(&mut rng, &d)));
