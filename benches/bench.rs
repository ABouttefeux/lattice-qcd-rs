...
use lattice_qcd_rs::{
    ...
    integrator::{SymplecticEuler, SymplecticEulerRayon},
    simulation::{
...
fn matrix_exp_old(rng: &mut ThreadRng, d: &impl Distribution<Real>) {
    let _m = (Su3Adjoint::random(rng, d).to_matrix() * nalgebra::Complex::<Real>::i()).exp();
}

fn matrix_exp_i(rng: &mut ThreadRng, d: &impl Distribution<Real>) {
    let _m = su3_exp_i(Su3Adjoint::random(rng, d));
}

fn matrix_exp_r(rng: &mut ThreadRng, d: &impl Distribution<Real>) {
    let _m = su3_exp_r(Su3Adjoint::random(rng, d));
}

fn create_su3_adj(rng: &mut ThreadRng, d: &impl Distribution<Real>) {
    let _m = Su3Adjoint::random(rng, d);
}

fn create_vec(rng: &mut ThreadRng, d: &impl Distribution<Real>, size: usize) {
    let mut data = Vec::with_capacity(size);
    for _ in 0..size {
        data.push(nalgebra::Vector4::new(
...
fn create_hash_map(rng: &mut ThreadRng, d: &impl Distribution<Real>, size: usize) {
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let link_m = LinkMatrix::new(vec![nalgebra::Matrix3::<Complex>::identity(); 10_000]);
...
fn benchmark_base(c: &mut Criterion) {
    ...
    let sizes: [usize; 5] = [100, 1_000, 10_000, 100_000, 1_000_000];
    for size in sizes {
        groupe_collection.throughput(Throughput::Elements(size as u64));
        groupe_collection.bench_with_input(BenchmarkId::new("vec size", size), &size, |b, i| {
            b.iter(|| create_vec(&mut rng, &d, *i));
        });
        groupe_collection.bench_with_input(
            BenchmarkId::new("hash_map size", size),
            &size,
            |b, i| {
