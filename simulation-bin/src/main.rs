...
use lattice_qcd_rs::{
    field::*, integrator::*, lattice::*, simulation::*, su3::*, CMatrix3, Complex, Real,
};
use na::ComplexField;
use rand::RngCore;
...
fn main() {
    let items = vec![
        "Hybrid Monte Carlo",
        "Metropolis Hastings absolute",
        "Metropolis Hastings Delta",
        "MHD + HMC",
    ];
...
fn generate_state_with_logs<const D: usize>(rng: &mut impl rand::Rng) -> LatticeStateDefault<D> {
    ...
    spinner.set_style(
        ProgressStyle::default_spinner()
            .tick_chars("|/-\\")
            .template("{prefix:10} [{elapsed_precise}] [{spinner}]"),
    );
...
#[allow(dead_code)]
fn simulate_with_log_subblock<MC>(
    mut simulation: LatticeStateDefault<4>,
    mc: &mut MC,
    number_of_sims: u64,
) -> LatticeStateDefault<4>
where
...
fn sim_hmc() {
    ...
    println!(
        "initial plaquette average {}",
        simulation.average_trace_plaquette().unwrap()
    );
    ...
    let simulation = simulate_loop_with_input(
        simulation,
        &mut hmc,
        number_of_sims,
        1,
        &|sim,
          mc: &HybridMonteCarloDiagnostic<
            LatticeStateDefault<DIM_USE>,
            Xoshiro256PlusPlus,
    ...
    let simulation = simulate_loop_with_input(
        ...
        >| {
            ...
            format!(
                "A {:.6},P {:.2},R {}",
                average / 3_f64,
                mc.mcd().prob_replace_last(),
                mc.mcd().has_replace_last()
            )
        },
    );
...
fn sim_dmh() {
    ...
    let simulation = simulate_loop_with_input(
        ...
        number_of_sims,
        sub_block,
        &|sim: &LatticeStateDefault<DIM_USE>,
          mc: &MetropolisHastingsDeltaDiagnostic<Xoshiro256PlusPlus>| {
            let average = sim.average_trace_plaquette().unwrap().real();
...
fn sim_dmh_hmc() {
    ...
    let seed = rng_seeder.next_u64();
    println!(
        "Begining simulation MH Delta -> HMC with seed {:#08x}",
        seed
    );
    let mut rng = Xoshiro256PlusPlus::seed_from_u64(seed);
    let simulation: LatticeStateDefault<DIM_USE> = generate_state_with_logs(&mut rng);
    ...
    let simulation = simulate_loop_with_input(
        ...
        sub_block,
        &|sim: &LatticeStateDefault<DIM_USE>,
          _mc: &MetropolisHastingsDeltaDiagnostic<Xoshiro256PlusPlus>| {
            //let average = sim.average_trace_plaquette().unwrap().real();
            let vec = sim
                .lattice()
                .get_points()
            ...
            let a = sim.beta().powi(4)
                * ((sum / number_of_plaquette)
                    - c1 / sim.beta()
                    - c2 / sim.beta().powi(2)
                    - c3 / sim.beta().powi(3)
                    - c4 * sim.beta().ln() / sim.beta().powi(4));
            format!(
                "A {:.6},  {}",
                a,
                lattice_qcd_rs::statistics::covariance(&initial_data, &vec)
                    .unwrap()
                    .abs()
            )
        },
